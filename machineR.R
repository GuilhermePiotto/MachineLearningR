#   Prevendo a Inadimplência de Clientes com Machine Learning e Linguagem R

# Definindo a pasta de trabalho
setwd('C:/Users/guilh/Desktop/powerbi/cap15')
getwd()
...
# Carregando os dados
dados_clientes <- read.csv('C:/Users/guilh/Desktop/Projeto_Oper/dataset.csv')

# visualizando os dados
# Data Set trazido de https://archive.ics.uci.edu/ml/datasets/default+of+credit+card+clients
View(dados_clientes)
dim(dados_clientes)
str(dados_clientes) 
...
colnames(dados_clientes)[1] <- "Saldo_Cartao"
...
# Se eu precisa-se retirar os valores ausentes
...
# Convertendo a variável idade para o tipo fator com faixa etária
...
sapply(dados_clientes, function(x) sum(is.na(x)))
missmap(dados_clientes, main = "Valores Missing Observados")
# Agora vou remover os valores ausentes
dados_clientes <- na.omit(dados_clientes)
missmap(dados_clientes, main = "Valores Missing Observados")
dim(dados_clientes)
View(dados_clientes)
...
# Vou ver a diferença entre as duas classes de inadimpletes
table(dados_clientes$inadimplente)
# 0 = pessoas que não ficaram inadimplentes
# 1 = pessoas que ficaram inadimplentes
...
# Defini os dados de treinamento como subconjunto do conjunto de dados original
...
# Observe o sinal - (menos) que indica que vou pegar todos os valores de indice
# que não foram utilizados nos dados de treinamento.
dados_teste <- dados_clientes[-indice,]
dim(dados_teste)
dim(dados_treino)

# Modelo de Machine Learning utilizando o random Forest
...
#########  Aqui começei com o SMOTE, porém quando atualizou o R para 4.2 parou
#          funcionar, então mais a baixo utilizei outra estratégia.
...
set.seed(9560)
?glm
dados_treino_bal <- recipe(inadimplente ~ ., data  = dados_treino) %>%
  themis::step_upsample(inadimplente) %>%
  prep() %>%
  juice()
table(dados_treino_bal$inadimplente)
prop.table(table(dados_treino_bal$inadimplente))
